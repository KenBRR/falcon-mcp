...
class TestDynamicToolCatalog(unittest.TestCase):
    ...
    def test_rtr_admin_tools_are_discoverable_and_executable(self):
        """Verify RTR Admin participates in dynamic search and execute dispatch."""
        module = RTRAdminModule(self.mock_client)
        dynamic = DynamicMode({"rtr_admin": module}, MagicMock())

        search_results = dynamic.catalog.search(query="classify", module="rtr_admin")
        tool_names = {result["name"] for result in search_results}
        self.assertIn("falcon_classify_rtr_admin_command", tool_names)

        result = run_async(
            dynamic._execute_tool(
                tool_name="falcon_classify_rtr_admin_command",
                parameters={"base_command": "ps"},
            )
        )

        self.assertEqual(result["base_command"], "ps")
